...
export const useChatStore = defineStore('chat', () => {
  ...
  const addPendingMessage = (conversationUUID, messageText, authorType, authorId, files = []) => {
    ...
    // Reuse the current user's avatar so the message bubble immediately shows the right avatar
    // instead of flashing the fallback letter before the server response arrives.
    let userAvatar = userStore.avatar || ''
    if (!userAvatar) {
      const existing = messageCache
        .getAllPagesMessages(conversationUUID)
        .find((m) => m.author?.id === authorId && (m.author?.avatar_url || ''))
      userAvatar = existing?.author?.avatar_url || ''
    }
