...
func handleChatSendMessage(r *fastglue.Request) error {
	...
	var botMessages []cmodels.Message
	if message, botMessages, err = app.conversation.ProcessIncomingLiveChatMessage(message); err != nil {
		app.lo.Error("error processing incoming message", "conversation_uuid", conversationUUID, "error", err)
		return r.SendErrorEnvelope(fasthttp.StatusInternalServerError, app.i18n.T("globals.messages.errorSendingMessage"), nil, envelope.GeneralError)
	}

	return sendChatMessageResponseWithBotMessages(app, r, message.UUID, botMessages)
}
...
// sendChatMessageResponseWithBotMessages sends the user message along with any
// bot messages generated by quick reply processing.
func sendChatMessageResponseWithBotMessages(app *App, r *fastglue.Request, messageUUID string, botMessages []cmodels.Message) error {
	...
	// Build bot messages for the response.
	var botMsgs []cmodels.ChatMessage
	for _, bm := range botMessages {
		app.conversation.SignAvatarURL(&bm.Author.AvatarURL)
		bmAuthor := bm.Author
		bmAuthor.Email = null.String{}
		botMsgs = append(botMsgs, cmodels.ChatMessage{
			UUID:             bm.UUID,
			CreatedAt:        bm.CreatedAt,
			Content:          bm.Content,
			TextContent:      bm.TextContent,
			ConversationUUID: bm.ConversationUUID,
			Status:           bm.Status,
			Author:           bmAuthor,
