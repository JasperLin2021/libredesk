...
func main() {
	...
	var (
		...
		noReplyTimeout              = cmp.Or(ko.Duration("conversation.no_reply_timeout"), 5*time.Minute)
		noReplyTimeoutScanInterval  = cmp.Or(ko.Duration("conversation.no_reply_timeout_scan_interval"), 30*time.Second)
	...
	go conversation.RunNoReplyTimeoutRefresher(ctx, noReplyTimeout, noReplyTimeoutScanInterval)
